    ...
    for(int i=0; i<IMG_ROWS; i++)
    {
		for(int j=0; j<IMG_COLS; j++)
		{
			...
			else
			{
				...
				output_ARM[i*IMG_COLS+j] = x_dir + (x_dir << 8) + (x_dir << 16);
...
printf("Gradient execution time on ARM: %ld \n",elapsed_secs_l);
...
printf("data transfer CPU to FPGA : %ld \n",elapsed_secs_l);	
  ...
  checkError(status, "Error: could not set Gradient threshold");

  cl_event event;

gettimeofday(&begin_l, 0); 
 
  status = clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &gradientSize, &gradientSize, 0, NULL, &event);

  //checkError(status, "Error: could not enqueue gradient filter");
...
/*  char title[256];
  sprintf(title, "Gradient Filter (%d FPS)", fps_raw);
  */
}
...
int main()
{
	imageFilename = "sample_image_10.jpg";
	...
	*/
	...
	for(int i=0;i<IMG_ROWS;i++)
	{
		for(int j=0;j<IMG_COLS;j++)
		{
			out_ptr[i*IMG_COLS+j] = output[i*IMG_COLS+j] & 0xFF;
	...
	*/
	...
	for(int i=0;i<IMG_ROWS;i++)
	{
		for(int j=0;j<IMG_COLS;j++)
		{
			out_ARM_ptr[i*IMG_COLS+j] = output_ARM[i*IMG_COLS+j] & 0xFF;
...
void initCL()
{
  ...
  std::string binary_file = getBoardBinaryFile("gradient", device);
  ...
  kernel = clCreateKernel(program, "gradient", &status);
  checkError(status, "Error: could not create gradient kernel");
  ...
  checkError(status, "Error: could not set gradient arg 0");
  status = clSetKernelArg(kernel, 1, sizeof(cl_mem), &out_buffer);
  checkError(status, "Error: could not set gradient arg 1");

  status = clSetKernelArg(kernel, 2, sizeof(int), &pixels);
  checkError(status, "Error: could not set gradient arg 2");
